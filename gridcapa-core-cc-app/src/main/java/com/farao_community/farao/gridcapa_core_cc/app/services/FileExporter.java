...
import com.farao_community.farao.gridcapa_core_cc.app.services.results_export.RexResultFileExporter;
...
import com.farao_community.farao.gridcapa_core_cc.app.services.results_export.MainResultFileExporter;
import com.farao_community.farao.gridcapa_core_cc.app.services.results_export.RemedialActionsFileExporter;
...
public class FileExporter {
    ...
    public Map<ResultType, String> exportStudyPointResult(List<StudyPointResult> studyPointResults, CoreCCRequest coreCCRequest, FbConstraintCreationContext cracCreationContext) {
        Map<ResultType, String> resultMap = new EnumMap<>(ResultType.class);
        if (coreCCRequest.getLaunchedAutomatically()) {
            resultMap.put(ResultType.MAIN_RESULT, mainResultFileExporter.exportStudyPointResult(studyPointResults, coreCCRequest.getTimestamp()));
        }
        resultMap.put(ResultType.REX_RESULT, rexResultFileExporter.exportStudyPointResult(studyPointResults, coreCCRequest.getTimestamp()));
        resultMap.put(ResultType.REMEDIAL_ACTIONS_RESULT, remedialActionsFileExporter.exportStudyPointResult(studyPointResults, coreCCRequest.getTimestamp(), cracCreationContext));
        return resultMap;
    }
    //endregion
    ...
    public String saveShiftedCgm(Network network, StudyPoint studyPoint) {
        ...
        try (InputStream is = memDataSource.newInputStream("", "xiidm")) {
            ...
            throw new CoreCCInternalException("Error while trying to save shifted network", e);
        }
    ...
    public String saveShiftedCgmWithPra(Network network, String filename) {
        ...
        try (InputStream is = memDataSource.newInputStream("", "uct")) {
            ...
            throw new CoreCCInternalException("Error while trying to save shifted network with pra", e);
        }
        return minioAdapter.generatePreSignedUrl(networkPath);
    }
    //endregion
    ...
    public String saveCracInJsonFormat(Crac crac, OffsetDateTime timestamp) {
        ...
        try (OutputStream os = memDataSource.newOutputStream(jsonCracFileName, false)) {
            ...
            throw new CoreCCInternalException("Error while trying to save converted CRAC file.", e);
        }
        ...
        try (InputStream is = memDataSource.newInputStream(jsonCracFileName)) {
            ...
            throw new CoreCCInternalException("Error while trying to upload converted CRAC file.", e);
        }
