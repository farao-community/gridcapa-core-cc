...
public final class IntervalUtil {
    ...
    public static int getInstantPosition(String instant, String interval) {
        ...
        return positionsMap.entrySet().stream().filter(entry -> entry.getValue().contains(Instant.parse(instant))).findFirst().orElseThrow(() -> new CoreCCInternalException(String.format("Instant %s not found in interval %s", instant, interval))).getKey();
    }
