    ...
    public static FbConstraintCreationContext importCracXmlGetFbInfoWithNetwork(String timestamp, Network network, InputStream cracXmlFileInputStream) {
        try {
            ...
            if (cracCreationContext.isCreationSuccessful()) {
                ...
                throw new CoreCCInternalException("Crac creation context failed for timestamp: " + timestamp);
            }
        } catch (Exception e) {
            throw new CoreCCInternalException("Crac creation context failed, timestamp: " + timestamp, e);
        }
    }
}
