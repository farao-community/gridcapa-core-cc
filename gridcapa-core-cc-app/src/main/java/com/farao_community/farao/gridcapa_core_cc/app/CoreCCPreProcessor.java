...
import com.rte_france.farao.rao_integration.server.constants.InputsNamingRules;
import com.rte_france.farao.rao_integration.server.entities.HourlyRaoRequest;
import com.rte_france.farao.rao_integration.server.entities.HourlyRaoResult;
import com.rte_france.farao.rao_integration.server.entities.RaoIntegrationTask;
import com.rte_france.farao.rao_integration.server.entities.TaskStatus;
import com.rte_france.farao.rao_integration.server.exceptions.RaoIntegrationException;
import com.rte_france.farao.rao_integration.server.inputs.rao_request.RequestMessage;
import com.rte_france.farao.rao_integration.server.inputs.rao_response.Header;
import com.rte_france.farao.rao_integration.server.inputs.rao_response.Reply;
import com.rte_france.farao.rao_integration.server.inputs.rao_response.ResponseMessage;
import com.rte_france.farao.rao_integration.server.messaging.MinioAdapter;
import com.rte_france.farao.rao_integration.server.postprocessing.CracHelper;
import com.rte_france.farao.rao_integration.server.postprocessing.OutputFileNameUtil;
import com.rte_france.farao.rao_integration.server.postprocessing.RaoIXmlResponseGenerator;
import com.rte_france.farao.rao_integration.server.util.CoreNetworkImporterWrapper;
import com.rte_france.farao.rao_integration.server.util.IntervalUtil;
import com.rte_france.farao.rao_integration.server.util.JaxbUtil;
import com.rte_france.farao.rao_integration.server.util.ZipUtil;
 ...
 */
@Service
public class CoreCCPreProcessor {
    private static final Logger LOGGER = LoggerFactory.getLogger(CoreCCPreProcessor.class);
    ...
    private void splitRaoRequest(CoreCCRequest coreCCRequest, MinioAdapter minioAdapter) {
        Set<RaoRequest> raoRequests = new HashSet<>();
        Set<RaoResult> raoResults = new HashSet<>();
        try {
            FileAttribute<Set<PosixFilePermission>> attr = PosixFilePermissions.asFileAttribute(PosixFilePermissions.fromString("rwx------"));
            String tmpInputsPath = Files.createTempDirectory("gridcapa-core-cc-temp-dir", attr).toString();
            String tmpCgmInputsPath = Files.createDirectories(Paths.get(tmpInputsPath + File.separator + "cgm"), attr).toString();

            ZipUtil.unzipFile();

            Path cgmsZipP = minioAdapter.copyFileInTargetSystemPath(cgmsZipMinioObjectName, targetTempPath);

            Path cgmsUnZippedTempFolderPath = Paths.get(tmpCgmInputsPath); //NOSONAR
    ...
    private void splitRaoRequestWhenError(RaoIntegrationTask raoIntegrationTask, Path tempInputPath, String exceptionMessage) {
        ...
        raoRequestMessage.getPayload().getRequestItems().getRequestItem().forEach(requestItem -> {
            ...
            raoResult.setStatus(TaskStatus.ERROR);
            raoResult.setErrorCode(HourlyRaoResult.ErrorCode.BD_PREPROCESSING_FAILURE);
    ...
    private String uploadJsonCrac(RaoIntegrationTask raoIntegrationTask, String destinationKey, Instant utcInstant, Path cracFileTmpPath, Network network) {
        try (InputStream cracIs = new BufferedInputStream(new FileInputStream(cracFileTmpPath.toFile()));
            ...
            CracCreationContext cracCreationContext = CracCreators.createCrac(nativeCrac, network, OffsetDateTime.parse(utcInstant.toString()), CracHelper.getCracCreationParameters());
